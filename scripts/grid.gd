	...
	for i in current_matches.size():
		...
		for j in current_matches.size():
			...
			if all_pieces[this_column][this_row] == null:
				continue
			if this_column == current_column and all_pieces[this_column][this_row].color == current_color:
				col_matched += 1
			if this_row == current_row and all_pieces[this_column][this_row].color == current_color:
				row_matched += 1
